...
JUICE_INTERNAL char *juice_stringf(allocator_t *alloc, const char *format, ...);
JUICE_INTERNAL void juice_log(const char *file, int line, const char *format, ...);
JUICE_INTERNAL char* juice_strndup(allocator_t *alloc, const char *string, size_t n);
JUICE_INTERNAL char* juice_strdup(allocator_t *alloc, const char *string);

JUICE_INTERNAL uint64_t juice_double_to_uint64(double val);
JUICE_INTERNAL double juice_uint64_to_double(uint64_t val);

JUICE_INTERNAL bool juice_timer_platform_supported(void);
JUICE_INTERNAL juice_timer_t juice_timer_start(void);
JUICE_INTERNAL double juice_timer_get_elapsed_ms(const juice_timer_t *timer);
...
JUICE_INTERNAL void errors_init(errors_t *errors);
JUICE_INTERNAL void errors_deinit(errors_t *errors);

JUICE_INTERNAL void errors_add_error(errors_t *errors, error_type_t type, src_pos_t pos, const char *message);
JUICE_INTERNAL void errors_add_errorf(errors_t *errors, error_type_t type, src_pos_t pos, const char *format, ...) __attribute__ ((format (printf, 4, 5)));
JUICE_INTERNAL void errors_clear(errors_t *errors);
JUICE_INTERNAL int errors_get_count(const errors_t *errors);
JUICE_INTERNAL error_t *errors_get(errors_t *errors, int ix);
JUICE_INTERNAL const error_t *errors_getc(const errors_t *errors, int ix);
JUICE_INTERNAL const char *error_type_to_string(error_type_t type);
JUICE_INTERNAL error_t *errors_get_last_error(errors_t *errors);
JUICE_INTERNAL bool errors_has_errors(const errors_t *errors);
...
JUICE_INTERNAL void token_init(token_t *tok, token_type_t type, const char *literal, int len); // no need to destroy
JUICE_INTERNAL char *token_duplicate_literal(allocator_t *alloc, const token_t *tok);
JUICE_INTERNAL const char *token_type_to_string(token_type_t type);
...
JUICE_INTERNAL compiled_file_t* compiled_file_make(allocator_t *alloc, const char *path);
JUICE_INTERNAL void compiled_file_destroy(compiled_file_t *file);
...
JUICE_INTERNAL bool lexer_init(lexer_t *lex, allocator_t *alloc, errors_t *errs, const char *input, compiled_file_t *file); // no need to deinit

JUICE_INTERNAL bool lexer_failed(lexer_t *lex);
JUICE_INTERNAL void lexer_continue_template_string(lexer_t *lex);
JUICE_INTERNAL bool lexer_cur_token_is(lexer_t *lex, token_type_t type);
JUICE_INTERNAL bool lexer_peek_token_is(lexer_t *lex, token_type_t type);
JUICE_INTERNAL bool lexer_next_token(lexer_t *lex);
JUICE_INTERNAL bool lexer_previous_token(lexer_t *lex);
JUICE_INTERNAL token_t lexer_next_token_internal(lexer_t *lex); // exposed here for tests
JUICE_INTERNAL bool lexer_expect_current(lexer_t *lex, token_type_t type);
...
JUICE_INTERNAL char* statements_to_string(allocator_t *alloc, ptrarray(statement_t) *statements);

JUICE_INTERNAL statement_t* statement_make_define(allocator_t *alloc, ident_t *name, expression_t *value, bool assignable);
JUICE_INTERNAL statement_t* statement_make_if(allocator_t *alloc, ptrarray(if_case_t) *cases, code_block_t *alternative);
JUICE_INTERNAL statement_t* statement_make_return(allocator_t *alloc, expression_t *value);
JUICE_INTERNAL statement_t* statement_make_expression(allocator_t *alloc, expression_t *value);
JUICE_INTERNAL statement_t* statement_make_while_loop(allocator_t *alloc, expression_t *test, code_block_t *body);
JUICE_INTERNAL statement_t* statement_make_break(allocator_t *alloc);
JUICE_INTERNAL statement_t* statement_make_foreach(allocator_t *alloc, ident_t *iterator, expression_t *source, code_block_t *body);
JUICE_INTERNAL statement_t* statement_make_for_loop(allocator_t *alloc, statement_t *init, expression_t *test, expression_t *update, code_block_t *body);
JUICE_INTERNAL statement_t* statement_make_continue(allocator_t *alloc);
JUICE_INTERNAL statement_t* statement_make_block(allocator_t *alloc, code_block_t *block);
JUICE_INTERNAL statement_t* statement_make_import(allocator_t *alloc, char *path);
JUICE_INTERNAL statement_t* statement_make_recover(allocator_t *alloc, ident_t *error_ident, code_block_t *body);

JUICE_INTERNAL void statement_destroy(statement_t *stmt);

JUICE_INTERNAL statement_t* statement_copy(const statement_t *stmt);

JUICE_INTERNAL code_block_t* code_block_make(allocator_t *alloc, ptrarray(statement_t) *statements);
JUICE_INTERNAL void code_block_destroy(code_block_t *stmt);
JUICE_INTERNAL code_block_t* code_block_copy(code_block_t *block);

JUICE_INTERNAL expression_t* expression_make_ident(allocator_t *alloc, ident_t *ident);
JUICE_INTERNAL expression_t* expression_make_number_literal(allocator_t *alloc, double val);
JUICE_INTERNAL expression_t* expression_make_bool_literal(allocator_t *alloc, bool val);
JUICE_INTERNAL expression_t* expression_make_string_literal(allocator_t *alloc, char *value);
JUICE_INTERNAL expression_t* expression_make_null_literal(allocator_t *alloc);
JUICE_INTERNAL expression_t* expression_make_array_literal(allocator_t *alloc, ptrarray(expression_t) *values);
JUICE_INTERNAL expression_t* expression_make_map_literal(allocator_t *alloc, ptrarray(expression_t) *keys, ptrarray(expression_t) *values);
JUICE_INTERNAL expression_t* expression_make_prefix(allocator_t *alloc, operator_t op, expression_t *right);
JUICE_INTERNAL expression_t* expression_make_infix(allocator_t *alloc, operator_t op, expression_t *left, expression_t *right);
JUICE_INTERNAL expression_t* expression_make_fn_literal(allocator_t *alloc, ptrarray(ident_t) *params, code_block_t *body);
JUICE_INTERNAL expression_t* expression_make_call(allocator_t *alloc, expression_t *function, ptrarray(expression_t) *args);
JUICE_INTERNAL expression_t* expression_make_index(allocator_t *alloc, expression_t *left, expression_t *index);
JUICE_INTERNAL expression_t* expression_make_assign(allocator_t *alloc, expression_t *dest, expression_t *source, bool is_postfix);
JUICE_INTERNAL expression_t* expression_make_logical(allocator_t *alloc, operator_t op, expression_t *left, expression_t *right);
JUICE_INTERNAL expression_t* expression_make_ternary(allocator_t *alloc, expression_t *test, expression_t *if_true, expression_t *if_false);

JUICE_INTERNAL void expression_destroy(expression_t *expr);

JUICE_INTERNAL expression_t* expression_copy(expression_t *expr);

JUICE_INTERNAL void statement_to_string(const statement_t *stmt, strbuf_t *buf);
JUICE_INTERNAL void expression_to_string(expression_t *expr, strbuf_t *buf);

JUICE_INTERNAL void code_block_to_string(const code_block_t *stmt, strbuf_t *buf);
JUICE_INTERNAL const char* operator_to_string(operator_t op);

JUICE_INTERNAL const char *expression_type_to_string(expression_type_t type);

JUICE_INTERNAL ident_t* ident_make(allocator_t *alloc, token_t tok);
JUICE_INTERNAL ident_t* ident_copy(ident_t *ident);
JUICE_INTERNAL void ident_destroy(ident_t *ident);

JUICE_INTERNAL if_case_t *if_case_make(allocator_t *alloc, expression_t *test, code_block_t *consequence);
JUICE_INTERNAL void if_case_destroy(if_case_t *cond);
JUICE_INTERNAL if_case_t* if_case_copy(if_case_t *cond);
...
JUICE_INTERNAL parser_t* parser_make(allocator_t *alloc, const juice_config_t *config, errors_t *errors);
JUICE_INTERNAL void parser_destroy(parser_t *parser);

JUICE_INTERNAL ptrarray(statement_t)* parser_parse_all(parser_t *parser,  const char *input, compiled_file_t *file);
...
JUICE_INTERNAL object_t object_make_from_data(object_type_t type, object_data_t *data);
JUICE_INTERNAL object_t object_make_number(double val);
JUICE_INTERNAL object_t object_make_bool(bool val);
JUICE_INTERNAL object_t object_make_null(void);
JUICE_INTERNAL object_t object_make_string(gcmem_t *mem, const char *string);
JUICE_INTERNAL object_t object_make_string_with_capacity(gcmem_t *mem, int capacity);
JUICE_INTERNAL object_t object_make_native_function(gcmem_t *mem, const char *name, native_fn fn, void *data, int data_len);
JUICE_INTERNAL object_t object_make_array(gcmem_t *mem);
JUICE_INTERNAL object_t object_make_array_with_capacity(gcmem_t *mem, unsigned capacity);
JUICE_INTERNAL object_t object_make_map(gcmem_t *mem);
JUICE_INTERNAL object_t object_make_map_with_capacity(gcmem_t *mem, unsigned capacity);
JUICE_INTERNAL object_t object_make_error(gcmem_t *mem, const char *message);
JUICE_INTERNAL object_t object_make_error_no_copy(gcmem_t *mem, char *message);
JUICE_INTERNAL object_t object_make_errorf(gcmem_t *mem, const char *fmt, ...) __attribute__ ((format (printf, 2, 3)));
JUICE_INTERNAL object_t object_make_function(gcmem_t *mem, const char *name, compilation_result_t *comp_res,
                                           bool owns_data, int num_locals, int num_args,
                                           int free_vals_count);
JUICE_INTERNAL object_t object_make_external(gcmem_t *mem, void *data);

JUICE_INTERNAL void object_deinit(object_t obj);
JUICE_INTERNAL void object_data_deinit(object_data_t *obj);

JUICE_INTERNAL bool        object_is_allocated(object_t obj);
JUICE_INTERNAL gcmem_t*    object_get_mem(object_t obj);
JUICE_INTERNAL bool        object_is_hashable(object_t obj);
JUICE_INTERNAL void        object_to_string(object_t obj, strbuf_t *buf, bool quote_str);
JUICE_INTERNAL const char* object_get_type_name(const object_type_t type);
JUICE_INTERNAL char*       object_get_type_union_name(allocator_t *alloc, const object_type_t type);
JUICE_INTERNAL char*       object_serialize(allocator_t *alloc, object_t object);
JUICE_INTERNAL object_t    object_deep_copy(gcmem_t *mem, object_t object);
JUICE_INTERNAL object_t    object_copy(gcmem_t *mem, object_t obj);
JUICE_INTERNAL double      object_compare(object_t a, object_t b, bool *out_ok);
JUICE_INTERNAL bool        object_equals(object_t a, object_t b);

JUICE_INTERNAL object_data_t* object_get_allocated_data(object_t object);

JUICE_INTERNAL bool           object_get_bool(object_t obj);
JUICE_INTERNAL double         object_get_number(object_t obj);
JUICE_INTERNAL function_t*    object_get_function(object_t obj);
JUICE_INTERNAL const char*    object_get_string(object_t obj);
JUICE_INTERNAL int            object_get_string_length(object_t obj);
JUICE_INTERNAL void           object_set_string_length(object_t obj, int len);
JUICE_INTERNAL int            object_get_string_capacity(object_t obj);
JUICE_INTERNAL char*          object_get_mutable_string(object_t obj);
JUICE_INTERNAL bool           object_string_append(object_t obj, const char *src, int len);
JUICE_INTERNAL unsigned long  object_get_string_hash(object_t obj);
JUICE_INTERNAL native_function_t* object_get_native_function(object_t obj);
JUICE_INTERNAL object_type_t  object_get_type(object_t obj);

JUICE_INTERNAL bool object_is_numeric(object_t obj);
JUICE_INTERNAL bool object_is_null(object_t obj);
JUICE_INTERNAL bool object_is_callable(object_t obj);

JUICE_INTERNAL const char* object_get_function_name(object_t obj);
JUICE_INTERNAL object_t    object_get_function_free_val(object_t obj, int ix);
JUICE_INTERNAL void        object_set_function_free_val(object_t obj, int ix, object_t val);
JUICE_INTERNAL object_t*   object_get_function_free_vals(object_t obj);

JUICE_INTERNAL const char*  object_get_error_message(object_t obj);
JUICE_INTERNAL void         object_set_error_traceback(object_t obj, traceback_t *traceback);
JUICE_INTERNAL traceback_t* object_get_error_traceback(object_t obj);

JUICE_INTERNAL external_data_t* object_get_external_data(object_t object);
JUICE_INTERNAL bool object_set_external_destroy_function(object_t object, external_data_destroy_fn destroy_fn);
JUICE_INTERNAL bool object_set_external_data(object_t object, void *data);
JUICE_INTERNAL bool object_set_external_copy_function(object_t object, external_data_copy_fn copy_fn);

JUICE_INTERNAL object_t object_get_array_value_at(object_t array, int ix);
JUICE_INTERNAL bool     object_set_array_value_at(object_t obj, int ix, object_t val);
JUICE_INTERNAL bool     object_add_array_value(object_t array, object_t val);
JUICE_INTERNAL int      object_get_array_length(object_t array);
JUICE_INTERNAL bool     object_remove_array_value_at(object_t array, int ix);

JUICE_INTERNAL int      object_get_map_length(object_t obj);
JUICE_INTERNAL object_t object_get_map_key_at(object_t obj, int ix);
JUICE_INTERNAL object_t object_get_map_value_at(object_t obj, int ix);
JUICE_INTERNAL bool     object_set_map_value_at(object_t obj, int ix, object_t val);
JUICE_INTERNAL object_t object_get_kv_pair_at(gcmem_t *mem, object_t obj, int ix);
JUICE_INTERNAL bool     object_set_map_value(object_t obj, object_t key, object_t val);
JUICE_INTERNAL object_t object_get_map_value(object_t obj, object_t key);
JUICE_INTERNAL bool     object_map_has_key(object_t obj, object_t key);
...
JUICE_INTERNAL global_store_t *global_store_make(allocator_t *alloc, gcmem_t *mem);
JUICE_INTERNAL void global_store_destroy(global_store_t *store);
JUICE_INTERNAL const symbol_t* global_store_get_symbol(global_store_t *store, const char *name);
JUICE_INTERNAL object_t global_store_get_object(global_store_t *store, const char *name);
JUICE_INTERNAL bool global_store_set(global_store_t *store, const char *name, object_t object);
JUICE_INTERNAL object_t global_store_get_object_at(global_store_t *store, int ix, bool *out_ok);
JUICE_INTERNAL bool global_store_set_object_at(global_store_t *store, int ix, object_t object);
JUICE_INTERNAL object_t *global_store_get_object_data(global_store_t *store);
JUICE_INTERNAL int global_store_get_object_count(global_store_t *store);
...
JUICE_INTERNAL symbol_t *symbol_make(allocator_t *alloc, const char *name, symbol_type_t type, int index, bool assignable);
JUICE_INTERNAL void symbol_destroy(symbol_t *symbol);
JUICE_INTERNAL symbol_t* symbol_copy(symbol_t *symbol);

JUICE_INTERNAL symbol_table_t *symbol_table_make(allocator_t *alloc, symbol_table_t *outer, global_store_t *global_store, int module_global_offset);
JUICE_INTERNAL void symbol_table_destroy(symbol_table_t *st);
JUICE_INTERNAL symbol_table_t* symbol_table_copy(symbol_table_t *st);
JUICE_INTERNAL bool symbol_table_add_module_symbol(symbol_table_t *st, symbol_t *symbol);
JUICE_INTERNAL const symbol_t *symbol_table_define(symbol_table_t *st, const char *name, bool assignable);
JUICE_INTERNAL const symbol_t *symbol_table_define_free(symbol_table_t *st, const symbol_t *original);
JUICE_INTERNAL const symbol_t *symbol_table_define_function_name(symbol_table_t *st, const char *name, bool assignable);
JUICE_INTERNAL const symbol_t *symbol_table_define_this(symbol_table_t *st);

JUICE_INTERNAL const symbol_t *symbol_table_resolve(symbol_table_t *st, const char *name);

JUICE_INTERNAL bool symbol_table_symbol_is_defined(symbol_table_t *st, const char *name);
JUICE_INTERNAL bool symbol_table_push_block_scope(symbol_table_t *table);
JUICE_INTERNAL void symbol_table_pop_block_scope(symbol_table_t *table);
JUICE_INTERNAL block_scope_t* symbol_table_get_block_scope(symbol_table_t *table);

JUICE_INTERNAL bool symbol_table_is_module_global_scope(symbol_table_t *table);
JUICE_INTERNAL bool symbol_table_is_top_block_scope(symbol_table_t *table);
JUICE_INTERNAL bool symbol_table_is_top_global_scope(symbol_table_t *table);

JUICE_INTERNAL int symbol_table_get_module_global_symbol_count(const symbol_table_t *table);
JUICE_INTERNAL const symbol_t * symbol_table_get_module_global_symbol_at(const symbol_table_t *table, int ix);
...
JUICE_INTERNAL opcode_definition_t* opcode_lookup(opcode_t op);
JUICE_INTERNAL const char *opcode_get_name(opcode_t op);
JUICE_INTERNAL int code_make(opcode_t op, int operands_count, uint64_t *operands, array(uint8_t) *res);
JUICE_INTERNAL void code_to_string(uint8_t *code, src_pos_t *source_positions, size_t code_size, strbuf_t *res);
JUICE_INTERNAL bool code_read_operands(opcode_definition_t *def, uint8_t *instr, uint64_t out_operands[2]);
...
JUICE_INTERNAL compilation_scope_t* compilation_scope_make(allocator_t *alloc, compilation_scope_t *outer);
JUICE_INTERNAL void compilation_scope_destroy(compilation_scope_t *scope);
JUICE_INTERNAL compilation_result_t *compilation_scope_orphan_result(compilation_scope_t *scope);

JUICE_INTERNAL compilation_result_t* compilation_result_make(allocator_t *alloc, uint8_t *bytecode, src_pos_t *src_positions, int count);
JUICE_INTERNAL void compilation_result_destroy(compilation_result_t* res);
...
JUICE_INTERNAL expression_t* optimise_expression(expression_t* expr);
...
typedef struct juice_config juice_config_t;
typedef struct gcmem gcmem_t;
...
JUICE_INTERNAL compiler_t *compiler_make(allocator_t *alloc,
                                       const juice_config_t *config,
                                       gcmem_t *mem, errors_t *errors,
                                       ptrarray(compiled_file_t) *files,
                                       global_store_t *global_store);
JUICE_INTERNAL void compiler_destroy(compiler_t *comp);
JUICE_INTERNAL compilation_result_t* compiler_compile(compiler_t *comp, const char *code);
JUICE_INTERNAL compilation_result_t* compiler_compile_file(compiler_t *comp, const char *path);
JUICE_INTERNAL symbol_table_t* compiler_get_symbol_table(compiler_t *comp);
JUICE_INTERNAL void compiler_set_symbol_table(compiler_t *comp, symbol_table_t *table);
JUICE_INTERNAL array(object_t)* compiler_get_constants(const compiler_t *comp);
...
JUICE_INTERNAL gcmem_t *gcmem_make(allocator_t *alloc);
JUICE_INTERNAL void gcmem_destroy(gcmem_t *mem);

JUICE_INTERNAL object_data_t* gcmem_alloc_object_data(gcmem_t *mem, object_type_t type);
JUICE_INTERNAL object_data_t* gcmem_get_object_data_from_pool(gcmem_t *mem, object_type_t type);

JUICE_INTERNAL void gc_unmark_all(gcmem_t *mem);
JUICE_INTERNAL void gc_mark_objects(object_t *objects, int count);
JUICE_INTERNAL void gc_mark_object(object_t object);
JUICE_INTERNAL void gc_sweep(gcmem_t *mem);

JUICE_INTERNAL bool gc_disable_on_object(object_t obj);
JUICE_INTERNAL void gc_enable_on_object(object_t obj);

JUICE_INTERNAL int gc_should_sweep(gcmem_t *mem);
...
JUICE_INTERNAL int builtins_count(void);
JUICE_INTERNAL native_fn builtins_get_fn(int ix);
JUICE_INTERNAL const char* builtins_get_name(int ix);
...
JUICE_INTERNAL traceback_t* traceback_make(allocator_t *alloc);
JUICE_INTERNAL void traceback_destroy(traceback_t *traceback);
JUICE_INTERNAL bool traceback_append(traceback_t *traceback, const char *function_name, src_pos_t pos);
JUICE_INTERNAL bool traceback_append_from_vm(traceback_t *traceback, vm_t *vm);
JUICE_INTERNAL bool traceback_to_string(const traceback_t *traceback, strbuf_t *buf);
JUICE_INTERNAL const char* traceback_item_get_line(traceback_item_t *item);
JUICE_INTERNAL const char* traceback_item_get_filepath(traceback_item_t *item);
...
JUICE_INTERNAL bool frame_init(frame_t* frame, object_t function, int base_pointer);

JUICE_INTERNAL opcode_val_t frame_read_opcode(frame_t* frame);
JUICE_INTERNAL uint64_t frame_read_uint64(frame_t* frame);
JUICE_INTERNAL uint16_t frame_read_uint16(frame_t* frame);
JUICE_INTERNAL uint8_t frame_read_uint8(frame_t* frame);
JUICE_INTERNAL src_pos_t frame_src_position(const frame_t *frame);
...
JUICE_INTERNAL vm_t* vm_make(allocator_t *alloc, const juice_config_t *config, gcmem_t *mem, errors_t *errors, global_store_t *global_store); // config can be null (for internal testing purposes)
JUICE_INTERNAL void  vm_destroy(vm_t *vm);

JUICE_INTERNAL void vm_reset(vm_t *vm);

JUICE_INTERNAL bool vm_run(vm_t *vm, compilation_result_t *comp_res, array(object_t) *constants);
JUICE_INTERNAL object_t vm_call(vm_t *vm, array(object_t) *constants, object_t callee, int argc, object_t *args);
JUICE_INTERNAL bool vm_execute_function(vm_t *vm, object_t function, array(object_t) *constants);

JUICE_INTERNAL object_t vm_get_last_popped(vm_t *vm);
JUICE_INTERNAL bool vm_has_errors(vm_t *vm);

JUICE_INTERNAL bool vm_set_global(vm_t *vm, int ix, object_t val);
JUICE_INTERNAL object_t vm_get_global(vm_t *vm, int ix);
...
JUICE_INTERNAL const src_pos_t src_pos_invalid = { NULL, -1, -1 };
JUICE_INTERNAL const src_pos_t src_pos_zero = { NULL, 0, 0 };
...
char *juice_stringf(allocator_t *alloc, const char *format, ...) {
    va_list args;
    ...
    (void)written;
    JUICE_ASSERT(written == to_write);
    va_end(args);
    return res;
}

void juice_log(const char *file, int line, const char *format, ...) {
    char msg[4096];
    int written = snprintf(msg, JUICE_ARRAY_LEN(msg), "%s:%d: ", file, line);
    (void)written;
    va_list args;
    va_start(args, format);
    int written_msg = vsnprintf(msg + written, JUICE_ARRAY_LEN(msg) - written, format, args);
    (void)written_msg;
    va_end(args);

    JUICE_ASSERT(written_msg <= (JUICE_ARRAY_LEN(msg) - written));

    printf("%s", msg);
}

char* juice_strndup(allocator_t *alloc, const char *string, size_t n) {
...
char* juice_strdup(allocator_t *alloc, const char *string) {
    if (!string) {
        return NULL;
    }
    return juice_strndup(alloc, string, strlen(string));
}

uint64_t juice_double_to_uint64(double val) {
    union {
        uint64_t val_uint64;
        double val_double;
    } temp = {
...
double juice_uint64_to_double(uint64_t val) {
    union {
        uint64_t val_uint64;
        double val_double;
    } temp = {
...
bool juice_timer_platform_supported() {
#if defined(JUICE_POSIX) || defined(JUICE_EMSCRIPTEN) || defined(JUICE_WINDOWS)
    return true;
#else
    return false;
#endif
...
juice_timer_t juice_timer_start() {
    juice_timer_t timer;
    memset(&timer, 0, sizeof(juice_timer_t));
#if defined(JUICE_POSIX)
...
double juice_timer_get_elapsed_ms(const juice_timer_t *timer) {
#if defined(JUICE_POSIX)
...
char* token_duplicate_literal(allocator_t *alloc, const token_t *tok) {
    return juice_strndup(alloc, tok->literal, tok->len);
}
...
compiled_file_t* compiled_file_make(allocator_t *alloc, const char *path) {
    ...
    if (last_slash_pos) {
        ...
        file->dir_path = juice_strndup(alloc, path, len);
    } else {
        file->dir_path = juice_strdup(alloc, "");
    }
    if (!file->dir_path) {
        goto error;
    }
    file->path = juice_strdup(alloc, path);
    if (!file->path) {
        goto error;
    }
...
static const char* read_number(lexer_t *lex, int *out_len) {
    ...
    int position = lex->position;
    while (is_digit(lex->ch) || is_one_of(lex->ch, allowed, JUICE_ARRAY_LEN(allowed) - 1)) {
        read_char(lex);
    }
...
static token_type_t lookup_identifier(const char *ident, int len) {
    ...
    for (int i = 0; i < JUICE_ARRAY_LEN(keywords); i++) {
        if (keywords[i].len == len && JUICE_STRNEQ(ident, keywords[i].value, len)) {
...
static bool add_line(lexer_t *lex, int offset) {
    ...
    char *line = NULL;
    if (!new_line_ptr) {
        line = juice_strdup(lex->alloc, line_start);
    } else {
        size_t line_len = new_line_ptr - line_start;
        line = juice_strndup(lex->alloc, line_start, line_len);
    }
    if (!line) {
        lex->failed = true;
        return false;
...
expression_t* expression_copy(expression_t *expr) {
    ...
    switch (expr->type) {
        ...
        case EXPRESSION_STRING_LITERAL: {
            char *string_copy = juice_strdup(expr->alloc, expr->string_literal);
            if (!string_copy) {
                return NULL;
            }
        ...
        case EXPRESSION_FUNCTION_LITERAL: {
            ...
            char *name_copy =  juice_strdup(expr->alloc, expr->fn_literal.name);
...
statement_t* statement_copy(const statement_t *stmt) {
    ...
    switch (stmt->type) {
        ...
        case STATEMENT_IMPORT: {
            char *path_copy = juice_strdup(stmt->alloc, stmt->import.path);
            if (!path_copy) {
                return NULL;
            }
...
ident_t* ident_copy(ident_t *ident) {
    ...
    res->value = juice_strdup(ident->alloc, ident->value);
    if (!res->value) {
...
static char escjuice_char(const char c);
...
parser_t* parser_make(allocator_t *alloc, const juice_config_t *config, errors_t *errors) {
...
static statement_t* parse_define_statement(parser_t *p) {
    ...
    if (value->type == EXPRESSION_FUNCTION_LITERAL) {
        value->fn_literal.name = juice_strdup(p->alloc, name_ident->value);
...
static expression_t* parse_assign_expression(parser_t *p, expression_t *left) {
    ...
    switch (assign_type) {
        ...
        default: JUICE_ASSERT(false); break;
    }
...
static char* process_and_copy_string(allocator_t *alloc, const char *input, size_t len) {
    ...
    while (input[in_i] != '\0' && in_i < len) {
        if (input[in_i] == '\\') {
            in_i++;
            output[out_i] = escjuice_char(input[in_i]);
            if (output[out_i] < 0) {
                goto error;
            }
        } else {
...
object_t global_store_get_object(global_store_t *store, const char *name) {
    ...
    JUICE_ASSERT(symbol->type == SYMBOL_JUICE_GLOBAL);
...
bool global_store_set(global_store_t *store, const char *name, object_t object) {
    ...
    symbol_t *symbol = symbol_make(store->alloc, name, SYMBOL_JUICE_GLOBAL, ix, false);
    if (!symbol) {
        goto err;
    }
...
symbol_t *symbol_make(allocator_t *alloc, const char *name, symbol_type_t type, int index, bool assignable) {
    ...
    symbol->alloc = alloc;
    symbol->name = juice_strdup(alloc, name);
    if (!symbol->name) {
...
const symbol_t *symbol_table_resolve(symbol_table_t *table, const char *name) {
    ...
    if (!symbol && table->outer) {
        ...
        if (symbol->type == SYMBOL_MODULE_GLOBAL || symbol->type == SYMBOL_JUICE_GLOBAL) {
            return symbol;
        }
        symbol = symbol_table_define_free(table, symbol);
    }
...
void code_to_string(uint8_t *code, src_pos_t *source_positions, size_t code_size, strbuf_t *res) {
    ...
    while (pos < code_size) {
        ...
        for (int i = 0; i < def->num_operands; i++) {
            if (op == OPCODE_NUMBER) {
                double val_double = juice_uint64_to_double(operands[i]);
...
static expression_t* optimise_infix_expression(expression_t* expr) {
    ...
    if (left_is_numeric && right_is_numeric) {
        ...
        switch (expr->infix.op) {
            ...
            case OPERATOR_EQ:       { res = expression_make_bool_literal(alloc, JUICE_DBLEQ(left_val, right_val)); break; }
            case OPERATOR_NOT_EQ:   { res = expression_make_bool_literal(alloc, !JUICE_DBLEQ(left_val, right_val)); break; }
        ...
        char *res_str = juice_stringf(alloc, "%s%s", left_val, right_val);
        if (res_str) {
...
compiler_t *compiler_make(allocator_t *alloc, const juice_config_t *config, gcmem_t *mem, errors_t *errors, ptrarray(compiled_file_t) *files, global_store_t *global_store) {
...
compilation_result_t* compiler_compile(compiler_t *comp, const char *code) {
    ...
    JUICE_ASSERT(array_count(comp->src_positions_stack) == 0);
    JUICE_ASSERT(array_count(compilation_scope->bytecode) == 0);
    JUICE_ASSERT(array_count(compilation_scope->break_ip_stack) == 0);
    JUICE_ASSERT(array_count(compilation_scope->continue_ip_stack) == 0);
    ...
    JUICE_ASSERT(compilation_scope->outer == NULL);
...
compilation_result_t* compiler_compile_file(compiler_t *comp, const char *path) {
    ...
    JUICE_ASSERT(ptrarray_count(comp->file_scopes) == 1);
...
static bool compiler_init_shallow_copy(compiler_t *copy, compiler_t *src) {
    ...
    JUICE_ASSERT(ptrarray_count(src->file_scopes) == 1);
    JUICE_ASSERT(src_st->outer == NULL);
    ...
    for (int i = 0; i < ptrarray_count(src_loaded_module_names); i++) {
        ...
        char *loaded_name_copy = juice_strdup(copy->alloc, loaded_name);
        if (!loaded_name_copy) {
            goto err;
        }
...
static int emit(compiler_t *comp, opcode_t op, int operands_count, uint64_t *operands) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        JUICE_ASSERT(src_pos->line >= 0);
        JUICE_ASSERT(src_pos->column >= 0);
...
static bool import_module(compiler_t *comp, const statement_t *import_stmt) { // todo: split into smaller functions
    ...
    if (kg_is_path_absolute(module_path)) {
        strbuf_appendf(filepath_buf, "%s.juice", module_path);
    } else {
        strbuf_appendf(filepath_buf, "%s%s.juice", file_scope->file->dir_path, module_path);
    }
    ...
    for (int i = 0; i < ptrarray_count(comp->file_scopes); i++) {
        ...
        if (JUICE_STREQ(fs->file->path, filepath)) {
    ...
    char *name_copy = juice_strdup(comp->alloc, module_name);
    if (!name_copy) {
        result = false;
        goto end;
    }
...
static bool compile_statement(compiler_t *comp, const statement_t *stmt) {
    ...
    switch (stmt->type) {
        ...
        case STATEMENT_FOREACH: {
            ...
            ip = emit(comp, OPCODE_NUMBER, 1, (uint64_t[]){juice_double_to_uint64(1)});
            if (ip < 0) {
                return false;
            }
...
static bool compile_expression(compiler_t *comp, expression_t *expr) {
    ...
    switch (expr->type) {
        ...
        case EXPRESSION_NUMBER_LITERAL: {
            ...
            ip = emit(comp, OPCODE_NUMBER, 1, (uint64_t[]){juice_double_to_uint64(number)});
            if (ip < 0) {
                goto error;
            }
...
static bool read_symbol(compiler_t *comp, const symbol_t *symbol) {
    ...
    } else if (symbol->type == SYMBOL_JUICE_GLOBAL) {
        ip = emit(comp, OPCODE_GET_JUICE_GLOBAL, 1, (uint64_t[]){symbol->index});
...
static module_t* module_make(allocator_t *alloc, const char *name) {
    ...
    module->alloc = alloc;
    module->name = juice_strdup(alloc, name);
    if (!module->name) {
        module_destroy(module);
        return NULL;
    }
...
static module_t* module_copy(module_t *src) {
    ...
    copy->name = juice_strdup(copy->alloc, src->name);
    if (!copy->name) {
        module_destroy(copy);
        return NULL;
    }
...
object_t object_make_native_function(gcmem_t *mem, const char *name, native_fn fn, void *data, int data_len) {
    ...
    obj->native_function.name = juice_strdup(mem->alloc, name);
...
object_t object_make_error(gcmem_t *mem, const char *error) {
    char *error_str = juice_strdup(mem->alloc, error);
    if (!error_str) {
        return object_make_null();
    }
...
object_t object_make_function(gcmem_t *mem, const char *name, compilation_result_t *comp_res, bool owns_data,
    ...
    if (owns_data) {
        data->function.name = name ? juice_strdup(mem->alloc, name) : juice_strdup(mem->alloc, "anonymous");
    ...
    if (free_vals_count >= JUICE_ARRAY_LEN(data->function.free_vals_buf)) {
...
char* object_get_type_union_name(allocator_t *alloc, const object_type_t type) {
    if (type == OBJECT_ANY || type == OBJECT_NONE || type == OBJECT_FREED) {
        return juice_strdup(alloc, object_get_type_name(type));
    }
    strbuf_t *res = strbuf_make(alloc);
    if (!res) {
        return NULL;
...
bool object_equals(object_t a, object_t b) {
    ...
    return JUICE_DBLEQ(res, 0);
}

external_data_t* object_get_external_data(object_t object) {
    JUICE_ASSERT(object_get_type(object) == OBJECT_EXTERNAL);
...
object_data_t* object_get_allocated_data(object_t object) {
    JUICE_ASSERT(object_is_allocated(object) || object_get_type(object) == OBJECT_NULL);
...
const char * object_get_string(object_t object) {
    JUICE_ASSERT(object_get_type(object) == OBJECT_STRING);
    object_data_t *data = object_get_allocated_data(object);
...
bool object_string_append(object_t obj, const char *src, int len) {
    JUICE_ASSERT(object_get_type(obj) == OBJECT_STRING);
    object_data_t *data = object_get_allocated_data(obj);
...
function_t* object_get_function(object_t object) {
    JUICE_ASSERT(object_get_type(object) == OBJECT_FUNCTION);
    object_data_t *data = object_get_allocated_data(object);
...
const char* object_get_function_name(object_t obj) {
    JUICE_ASSERT(object_get_type(obj) == OBJECT_FUNCTION);
    object_data_t *data = object_get_allocated_data(obj);
    JUICE_ASSERT(data);
    if (!data) {
        return NULL;
...
object_t object_get_function_free_val(object_t obj, int ix) {
    ...
    JUICE_ASSERT(ix >= 0 && ix < fun->free_vals_count);
...
const char* object_get_error_message(object_t object) {
    JUICE_ASSERT(object_get_type(object) == OBJECT_ERROR);
    object_data_t *data = object_get_allocated_data(object);
...
void object_set_error_traceback(object_t object, traceback_t *traceback) {
    ...
    object_data_t *data = object_get_allocated_data(object);
    JUICE_ASSERT(data->error.traceback == NULL);
...
object_t object_get_array_value_at(object_t object, int ix) {
    JUICE_ASSERT(object_get_type(object) == OBJECT_ARRAY);
...
JUICE_INTERNAL bool object_remove_array_value_at(object_t object, int ix) {
    array(object_t)* array = object_get_allocated_array(object);
    return array_remove_at(array, ix);
}

int object_get_map_length(object_t object) {
    JUICE_ASSERT(object_get_type(object) == OBJECT_MAP);
    object_data_t *data = object_get_allocated_data(object);
...
static bool freevals_are_allocated(function_t *fun) {
    return fun->free_vals_count >= JUICE_ARRAY_LEN(fun->free_vals_buf);
}

static char *object_data_get_string(object_data_t *data) {
    JUICE_ASSERT(data->type == OBJECT_STRING);
    if (data->string.is_allocated) {
...
static bool object_data_string_reserve_capacity(object_data_t *data, int capacity) {
    ...
    if (capacity <= (OBJECT_STRING_BUF_SIZE - 1)) {
        if (string->is_allocated) {
            JUICE_ASSERT(false); // should never happen
...
object_data_t* gcmem_alloc_object_data(gcmem_t *mem, object_type_t type) {
    ...
    JUICE_ASSERT(ptrarray_count(mem->objects_back) >= ptrarray_count(mem->objects));
...
int builtins_count() {
    return JUICE_ARRAY_LEN(g_native_functions);
}
...
static object_t println_fn(vm_t *vm, void *data, int argc, object_t *args) {
    ...
    const juice_config_t *config = vm->config;
...
bool traceback_append(traceback_t *traceback, const char *function_name, src_pos_t pos) {
    ...
    item.function_name = juice_strdup(traceback->alloc, function_name);
...
vm_t *vm_make(allocator_t *alloc, const juice_config_t *config, gcmem_t *mem, errors_t *errors, global_store_t *global_store) {
    ...
    while (vm->current_frame->ip < vm->current_frame->bytecode_size) {
        ...
        switch (opcode) {
            ...
            case OPCODE_GREATER_THAN_EQUAL:
            {
                ...
                bool res_val = false;
                switch (opcode) {
                    case OPCODE_EQUAL: res_val = JUICE_DBLEQ(comparison_res, 0); break;
                    case OPCODE_NOT_EQUAL: res_val = !JUICE_DBLEQ(comparison_res, 0); break;
                    case OPCODE_GREATER_THAN: res_val = comparison_res > 0; break;
                    case OPCODE_GREATER_THAN_EQUAL: {
                        res_val = comparison_res > 0 || JUICE_DBLEQ(comparison_res, 0);
                        break;
                    }
                    default: JUICE_ASSERT(false); break;
                }
            ...
            case OPCODE_NUMBER: {
                ...
                double val_double = juice_uint64_to_double(val);
        ...
        if (check_time) {
            ...
            if (time_check_counter > time_check_interval) {
                int elapsed_ms = (int)juice_timer_get_elapsed_ms(&timer);
...
static void stack_push(vm_t *vm, object_t obj) {
    ...
    if (vm->current_frame) {
        ...
        JUICE_ASSERT(vm->sp >= (frame->base_pointer + num_locals));
    }
#endif
    vm->stack[vm->sp] = obj;
    vm->sp++;
...
static object_t stack_pop(vm_t *vm) {
    ...
    if (vm->current_frame) {
        ...
        function_t *current_function = object_get_function(frame->function);
        int num_locals = current_function->num_locals;
        JUICE_ASSERT((vm->sp - 1) >= (frame->base_pointer + num_locals));
    }
#endif
    vm->sp--;
...
static object_t call_native_function(vm_t *vm, object_t callee, src_pos_t src_pos, int argc, object_t *args) {
    ...
    if (errors_has_errors(vm->errors) && !JUICE_STREQ(native_fun->name, "crash")) {
        error_t *err = errors_get_last_error(vm->errors);
        err->pos = src_pos;
        err->traceback = traceback_make(vm->alloc);
        if (err->traceback) {
    ...
    if (res_type == OBJECT_ERROR) {
        ...
        if (traceback) {
            ...
            if (!JUICE_STREQ(native_fun->name, "error")) {
...
#define JUICE_IMPL_VERSION_MAJOR 0
#define JUICE_IMPL_VERSION_MINOR 14
#define JUICE_IMPL_VERSION_PATCH 0
...
typedef struct juice_program {
    juice_t *juice;
    compilation_result_t *comp_res;
} juice_program_t;

typedef struct juice {
    allocator_t alloc;
    gcmem_t *mem;
    ptrarray(compiled_file_t) *files;
    global_store_t *global_store;
...
static void juice_deinit(juice_t *juice);
static object_t juice_native_fn_wrapper(vm_t *vm, void *data, int argc, object_t *args);
static object_t juice_object_to_object(juice_object_t obj);
static juice_object_t object_to_juice_object(object_t obj);
static juice_object_t juice_object_make_native_function_with_name(juice_t *juice, const char *name, juice_native_fn fn, void *data);

static void reset_state(juice_t *juice);
static void set_default_config(juice_t *juice);
...
static void* juice_malloc(void *ctx, size_t size);
static void juice_free(void *ctx, void *ptr);

juice_t *juice_make(void) {
    return juice_make_ex(NULL, NULL, NULL);
}
...
juice_t* juice_make_ex(juice_malloc_fn malloc_fn, juice_free_fn free_fn, void *ctx) {
    ...
    juice_t *juice = allocator_malloc(&custom_alloc, sizeof(juice_t));
    if (!juice) {
        return NULL;
    }

    memset(juice, 0, sizeof(juice_t));
    juice->alloc = allocator_make(juice_malloc, juice_free, juice);
    juice->custom_allocator = custom_alloc;

    set_default_config(juice);

    errors_init(&juice->errors);

    juice->mem = gcmem_make(&juice->alloc);
    if (!juice->mem) {
        goto err;
    }

    juice->files = ptrarray_make(&juice->alloc);
    if (!juice->files) {
        goto err;
    }

    juice->global_store = global_store_make(&juice->alloc, juice->mem);
    if (!juice->global_store) {
        goto err;
    }

    juice->compiler = compiler_make(&juice->alloc, &juice->config, juice->mem, &juice->errors, juice->files, juice->global_store);
    if (!juice->compiler) {
        goto err;
    }
    
    juice->vm = vm_make(&juice->alloc, &juice->config, juice->mem, &juice->errors, juice->global_store);
    if (!juice->vm) {
        goto err;
    }
...
err:
    juice_deinit(juice);
    allocator_free(&custom_alloc, juice);
    return NULL;
}
...
void juice_destroy(juice_t *juice) {
    ...
    juice_deinit(juice);
    allocator_t alloc = juice->alloc;
    allocator_free(&alloc, juice);
}

void juice_free_allocated(juice_t *juice, void *ptr) {
    allocator_free(&juice->alloc, ptr);
}

void juice_set_repl_mode(juice_t *juice, bool enabled) {
    juice->config.repl_mode = enabled;
}

bool juice_set_timeout(juice_t *juice, double max_execution_time_ms) {
    if (!juice_timer_platform_supported()) {
        juice->config.max_execution_time_ms = 0;
        juice->config.max_execution_time_set = false;
        return false;
    }

    if (max_execution_time_ms >= 0) {
        juice->config.max_execution_time_ms = max_execution_time_ms;
        juice->config.max_execution_time_set = true;
    } else {
        juice->config.max_execution_time_ms = 0;
        juice->config.max_execution_time_set = false;
    }
...
void juice_set_stdout_write_function(juice_t *juice, juice_stdout_write_fn stdout_write, void *context) {
    juice->config.stdio.write.write = stdout_write;
    juice->config.stdio.write.context = context;
}

void juice_set_file_write_function(juice_t *juice, juice_write_file_fn file_write, void *context) {
    juice->config.fileio.write_file.write_file = file_write;
    juice->config.fileio.write_file.context = context;
}

void juice_set_file_read_function(juice_t *juice, juice_read_file_fn file_read, void *context) {
    juice->config.fileio.read_file.read_file = file_read;
    juice->config.fileio.read_file.context = context;
}

juice_program_t* juice_compile(juice_t *juice, const char *code) {
    juice_clear_errors(juice);

    compilation_result_t *comp_res = NULL;

    comp_res = compiler_compile(juice->compiler, code);
    if (!comp_res || errors_get_count(&juice->errors) > 0) {
        goto err;
    }

    juice_program_t *program = allocator_malloc(&juice->alloc, sizeof(juice_program_t));
    if (!program) {
        goto err;
    }
    program->juice = juice;
...
juice_program_t* juice_compile_file(juice_t *juice, const char *path) {
    ...
    comp_res = compiler_compile_file(juice->compiler, path);
    if (!comp_res || errors_get_count(&juice->errors) > 0) {
        goto err;
    }
...
juice_object_t juice_execute_program(juice_t *juice, const juice_program_t *program) {
    ...
    if (juice != program->juice) {
        errors_add_error(&juice->errors, ERROR_USER, src_pos_invalid, "juice program was compiled with a different juice instance");
        return juice_object_make_null();
    }

    bool ok = vm_run(juice->vm, program->comp_res, compiler_get_constants(juice->compiler));
    if (!ok || errors_get_count(&juice->errors) > 0) {
        return juice_object_make_null();
    }

    JUICE_ASSERT(juice->vm->sp == 0);

    object_t res = vm_get_last_popped(juice->vm);
    if (object_get_type(res) == OBJECT_NONE) {
        return juice_object_make_null();
    }

    return object_to_juice_object(res);
}
...
void juice_program_destroy(juice_program_t *program) {
    ...
    allocator_free(&program->juice->alloc, program);
}
...
juice_object_t juice_execute(juice_t *juice, const char *code) {
    ...
    bool ok = vm_run(juice->vm, comp_res, compiler_get_constants(juice->compiler));
    if (!ok || errors_get_count(&juice->errors) > 0) {
        goto err;
    }
...
juice_object_t juice_call(juice_t *juice, const char *function_name, int argc, juice_object_t *args) {
    ...
    object_t callee = juice_object_to_object(juice_get_object(juice, function_name));
    if (object_get_type(callee) == OBJECT_NULL) {
        return juice_object_make_null();
    }
    object_t res = vm_call(juice->vm, compiler_get_constants(juice->compiler), callee, argc, (object_t*)args);
    if (errors_get_count(&juice->errors) > 0) {
        return juice_object_make_null();
    }
    return object_to_juice_object(res);
}
...
bool juice_has_errors(const juice_t *juice) {
    return juice_errors_count(juice) > 0;
}

int juice_errors_count(const juice_t *juice) {
    return errors_get_count(&juice->errors);
}

void juice_clear_errors(juice_t *juice) {
    errors_clear(&juice->errors);
}

const juice_error_t* juice_get_error(const juice_t *juice, int index) {
    return (const juice_error_t*)errors_getc(&juice->errors, index);
}

bool juice_set_native_function(juice_t *juice, const char *name, juice_native_fn fn, void *data) {
    juice_object_t obj = juice_object_make_native_function_with_name(juice, name, fn, data);
    if (juice_object_is_null(obj)) {
        return false;
    }
    return juice_set_global_constant(juice, name, obj);
}

bool juice_set_global_constant(juice_t *juice, const char *name, juice_object_t obj) {
    return global_store_set(juice->global_store, name, juice_object_to_object(obj));
}

juice_object_t juice_get_object(juice_t *juice, const char *name) {
    symbol_table_t *st = compiler_get_symbol_table(juice->compiler);
    const symbol_t *symbol = symbol_table_resolve(st, name);
    if (!symbol) {
        errors_add_errorf(&juice->errors, ERROR_USER, src_pos_invalid, "Symbol \"%s\" is not defined", name);
        return juice_object_make_null();
    }
    object_t res = object_make_null();
    if (symbol->type == SYMBOL_MODULE_GLOBAL) {
        res = vm_get_global(juice->vm, symbol->index);
    } else if (symbol->type == SYMBOL_JUICE_GLOBAL) {
        bool ok = false;
        res = global_store_get_object_at(juice->global_store, symbol->index, &ok);
        if (!ok) {
            errors_add_errorf(&juice->errors, ERROR_USER, src_pos_invalid, "Failed to get global object at %d", symbol->index);
            return juice_object_make_null();
        }
    } else {
        errors_add_errorf(&juice->errors, ERROR_USER, src_pos_invalid, "Value associated with symbol \"%s\" could not be loaded", name);
        return juice_object_make_null();
    }
    return object_to_juice_object(res);
}
...
bool juice_check_args(juice_t *juice, bool generate_error, int argc, juice_object_t *args, int expected_argc, int *expected_types) {
    if (argc != expected_argc) {
        if (generate_error) {
            juice_set_runtime_errorf(juice, "Invalid number or arguments, got %d instead of %d", argc, expected_argc);
        }
        return false;
    }
    
    for(int i = 0; i < argc; i++) {
        juice_object_t arg = args[i];
        juice_object_type_t type = juice_object_get_type(arg);
        juice_object_type_t expected_type = expected_types[i];
        if (!(type & expected_type)) {
            if (generate_error) {
                const char *type_str = juice_object_get_type_name(type);
                const char *expected_type_str = juice_object_get_type_name(expected_type);
                juice_set_runtime_errorf(juice, "Invalid argument type, got %s, expected %s", type_str, expected_type_str);
            }
            return false;
        }
    }
...
juice_object_t juice_object_make_number(double val) {
    return object_to_juice_object(object_make_number(val));
}

juice_object_t juice_object_make_bool(bool val) {
    return object_to_juice_object(object_make_bool(val));
}

juice_object_t juice_object_make_string(juice_t *juice, const char *str) {
    return object_to_juice_object(object_make_string(juice->mem, str));
}

juice_object_t juice_object_make_stringf(juice_t *juice, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    int to_write = vsnprintf(NULL, 0, fmt, args);
    va_end(args);
    va_start(args, fmt);
    object_t res = object_make_string_with_capacity(juice->mem, to_write);
    if (object_is_null(res)) {
        return juice_object_make_null();
    }
    char *res_buf = object_get_mutable_string(res);
...
juice_object_t juice_object_make_null() {
    return object_to_juice_object(object_make_null());
}

juice_object_t juice_object_make_array(juice_t *juice) {
    return object_to_juice_object(object_make_array(juice->mem));
}

juice_object_t juice_object_make_map(juice_t *juice) {
    return object_to_juice_object(object_make_map(juice->mem));
}

juice_object_t juice_object_make_native_function(juice_t *juice, juice_native_fn fn, void *data) {
    return juice_object_make_native_function_with_name(juice, "", fn, data);
}

juice_object_t juice_object_make_error(juice_t *juice, const char *msg) {
    return object_to_juice_object(object_make_error(juice->mem, msg));
}

juice_object_t juice_object_make_errorf(juice_t *juice, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    int to_write = vsnprintf(NULL, 0, fmt, args);
    va_end(args);
    va_start(args, fmt);
    char *res = (char*)allocator_malloc(&juice->alloc, to_write + 1);
    if (!res) {
        return juice_object_make_null();
    }
    int written = vsprintf(res, fmt, args);
    ...
    JUICE_ASSERT(written == to_write);
    va_end(args);
    return object_to_juice_object(object_make_error_no_copy(juice->mem, res));
}

juice_object_t juice_object_make_external(juice_t *juice, void *data) {
    object_t res = object_make_external(juice->mem, data);
    return object_to_juice_object(res);
}

char *juice_object_serialize(juice_t *juice, juice_object_t obj) {
    return object_serialize(&juice->alloc, juice_object_to_object(obj));
}

bool juice_object_disable_gc(juice_object_t juice_obj) {
    object_t obj = juice_object_to_object(juice_obj);
    return gc_disable_on_object(obj);
}

void juice_object_enable_gc(juice_object_t juice_obj) {
    object_t obj = juice_object_to_object(juice_obj);
    gc_enable_on_object(obj);
}

bool juice_object_equals(juice_object_t juice_a, juice_object_t juice_b){
    object_t a = juice_object_to_object(juice_a);
    object_t b = juice_object_to_object(juice_b);
    return object_equals(a, b);
}

bool juice_object_is_null(juice_object_t obj) {
    return juice_object_get_type(obj) == JUICE_OBJECT_NULL;
}

juice_object_t juice_object_copy(juice_object_t juice_obj) {
    object_t obj = juice_object_to_object(juice_obj);
...
juice_object_t juice_object_deep_copy(juice_object_t juice_obj) {
    object_t obj = juice_object_to_object(juice_obj);
    gcmem_t *mem = object_get_mem(obj);
...
void juice_set_runtime_error(juice_t *juice, const char *message) {
    errors_add_error(&juice->errors, ERROR_RUNTIME, src_pos_invalid, message);
}

void juice_set_runtime_errorf(juice_t *juice, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    int to_write = vsnprintf(NULL, 0, fmt, args);
    (void)to_write;
    ...
    JUICE_ASSERT(to_write == written);
    va_end(args);
    errors_add_error(&juice->errors, ERROR_RUNTIME, src_pos_invalid, res);
}

juice_object_type_t juice_object_get_type(juice_object_t juice_obj) {
    object_t obj = juice_object_to_object(juice_obj);
    switch (object_get_type(obj)) {
        case OBJECT_NONE:            return JUICE_OBJECT_NONE;
        case OBJECT_ERROR:           return JUICE_OBJECT_ERROR;
        case OBJECT_NUMBER:          return JUICE_OBJECT_NUMBER;
        case OBJECT_BOOL:            return JUICE_OBJECT_BOOL;
        case OBJECT_STRING:          return JUICE_OBJECT_STRING;
        case OBJECT_NULL:            return JUICE_OBJECT_NULL;
        case OBJECT_NATIVE_FUNCTION: return JUICE_OBJECT_NATIVE_FUNCTION;
        case OBJECT_ARRAY:           return JUICE_OBJECT_ARRAY;
        case OBJECT_MAP:             return JUICE_OBJECT_MAP;
        case OBJECT_FUNCTION:        return JUICE_OBJECT_FUNCTION;
        case OBJECT_EXTERNAL:        return JUICE_OBJECT_EXTERNAL;
        case OBJECT_FREED:           return JUICE_OBJECT_FREED;
        case OBJECT_ANY:             return JUICE_OBJECT_ANY;
        default:                     return JUICE_OBJECT_NONE;
    }
}

const char* juice_object_get_type_string(juice_object_t obj) {
    return juice_object_get_type_name(juice_object_get_type(obj));
}

const char* juice_object_get_type_name(juice_object_type_t type) {
    switch (type) {
        case JUICE_OBJECT_NONE:            return "NONE";
        case JUICE_OBJECT_ERROR:           return "ERROR";
        case JUICE_OBJECT_NUMBER:          return "NUMBER";
        case JUICE_OBJECT_BOOL:            return "BOOL";
        case JUICE_OBJECT_STRING:          return "STRING";
        case JUICE_OBJECT_NULL:            return "NULL";
        case JUICE_OBJECT_NATIVE_FUNCTION: return "NATIVE_FUNCTION";
        case JUICE_OBJECT_ARRAY:           return "ARRAY";
        case JUICE_OBJECT_MAP:             return "MAP";
        case JUICE_OBJECT_FUNCTION:        return "FUNCTION";
        case JUICE_OBJECT_EXTERNAL:        return "EXTERNAL";
        case JUICE_OBJECT_FREED:           return "FREED";
        case JUICE_OBJECT_ANY:             return "ANY";
        default:                         return "NONE";
    }
}

double juice_object_get_number(juice_object_t obj) {
    return object_get_number(juice_object_to_object(obj));
}

bool juice_object_get_bool(juice_object_t obj) {
    return object_get_bool(juice_object_to_object(obj));
}

const char *juice_object_get_string(juice_object_t obj) {
    return object_get_string(juice_object_to_object(obj));
}

const char *juice_object_get_error_message(juice_object_t obj) {
    return object_get_error_message(juice_object_to_object(obj));
}

const juice_traceback_t* juice_object_get_error_traceback(juice_object_t juice_obj) {
    object_t obj = juice_object_to_object(juice_obj);
    return (const juice_traceback_t*)object_get_error_traceback(obj);
}

bool juice_object_set_external_destroy_function(juice_object_t object, juice_data_destroy_fn destroy_fn) {
    return object_set_external_destroy_function(juice_object_to_object(object), (external_data_destroy_fn)destroy_fn);
}

bool juice_object_set_external_copy_function(juice_object_t object, juice_data_copy_fn copy_fn) {
    return object_set_external_copy_function(juice_object_to_object(object), (external_data_copy_fn)copy_fn);
}

int juice_object_get_array_length(juice_object_t obj) {
    return object_get_array_length(juice_object_to_object(obj));
}

juice_object_t juice_object_get_array_value(juice_object_t obj, int ix) {
    object_t res = object_get_array_value_at(juice_object_to_object(obj), ix);
    return object_to_juice_object(res);
}

const char* juice_object_get_array_string(juice_object_t obj, int ix) {
    juice_object_t object = juice_object_get_array_value(obj, ix);
    if (juice_object_get_type(object) != JUICE_OBJECT_STRING) {
        return NULL;
    }
    return juice_object_get_string(object);
}

double juice_object_get_array_number(juice_object_t obj, int ix) {
    juice_object_t object = juice_object_get_array_value(obj, ix);
    if (juice_object_get_type(object) != JUICE_OBJECT_NUMBER) {
        return 0;
    }
    return juice_object_get_number(object);
}

bool juice_object_get_array_bool(juice_object_t obj, int ix) {
    juice_object_t object = juice_object_get_array_value(obj, ix);
    if (juice_object_get_type(object) != JUICE_OBJECT_BOOL) {
        return 0;
    }
    return juice_object_get_bool(object);
}

bool juice_object_set_array_value(juice_object_t juice_obj, int ix, juice_object_t juice_value) {
    object_t obj = juice_object_to_object(juice_obj);
    object_t value = juice_object_to_object(juice_value);
    return object_set_array_value_at(obj, ix, value);
}

bool juice_object_set_array_string(juice_object_t obj, int ix, const char *string) {
    gcmem_t *mem = object_get_mem(juice_object_to_object(obj));
    if (!mem) {
        return false;
    }
    object_t new_value = object_make_string(mem, string);
    return juice_object_set_array_value(obj, ix, object_to_juice_object(new_value));
}

bool juice_object_set_array_number(juice_object_t obj, int ix, double number) {
    object_t new_value = object_make_number(number);
    return juice_object_set_array_value(obj, ix, object_to_juice_object(new_value));
}

bool juice_object_set_array_bool(juice_object_t obj, int ix, bool value) {
    object_t new_value = object_make_bool(value);
    return juice_object_set_array_value(obj, ix, object_to_juice_object(new_value));
}

bool juice_object_add_array_value(juice_object_t juice_obj, juice_object_t juice_value) {
    object_t obj = juice_object_to_object(juice_obj);
    object_t value = juice_object_to_object(juice_value);
    return object_add_array_value(obj, value);
}

bool juice_object_add_array_string(juice_object_t obj, const char *string) {
    gcmem_t *mem = object_get_mem(juice_object_to_object(obj));
    if (!mem) {
        return false;
    }
    object_t new_value = object_make_string(mem, string);
    return juice_object_add_array_value(obj, object_to_juice_object(new_value));
}

bool juice_object_add_array_number(juice_object_t obj, double number) {
    object_t new_value = object_make_number(number);
    return juice_object_add_array_value(obj, object_to_juice_object(new_value));
}

bool juice_object_add_array_bool(juice_object_t obj, bool value) {
    object_t new_value = object_make_bool(value);
    return juice_object_add_array_value(obj, object_to_juice_object(new_value));
}

int juice_object_get_map_length(juice_object_t obj) {
    return object_get_map_length(juice_object_to_object(obj));
}

juice_object_t juice_object_get_map_key_at(juice_object_t juice_obj, int ix) {
    object_t obj = juice_object_to_object(juice_obj);
    return object_to_juice_object(object_get_map_key_at(obj, ix));
}

juice_object_t juice_object_get_map_value_at(juice_object_t juice_obj, int ix) {
    object_t obj = juice_object_to_object(juice_obj);
    object_t res = object_get_map_value_at(obj, ix);
    return object_to_juice_object(res);
}

bool juice_object_set_map_value_at(juice_object_t juice_obj, int ix, juice_object_t juice_val) {
    object_t obj = juice_object_to_object(juice_obj);
    object_t val = juice_object_to_object(juice_val);
    return object_set_map_value_at(obj, ix, val);
}

bool juice_object_set_map_value_with_value_key(juice_object_t obj, juice_object_t key, juice_object_t val) {
    return object_set_map_value(juice_object_to_object(obj), juice_object_to_object(key), juice_object_to_object(val));
}

bool juice_object_set_map_value(juice_object_t obj, const char *key, juice_object_t value) {
    gcmem_t *mem = object_get_mem(juice_object_to_object(obj));
    if (!mem) {
        return false;
    }
    ...
    return juice_object_set_map_value_with_value_key(obj, object_to_juice_object(key_object), value);
}

bool juice_object_set_map_string(juice_object_t obj, const char *key, const char *string) {
    gcmem_t *mem = object_get_mem(juice_object_to_object(obj));
    if (!mem) {
        return false;
    }
    ...
    return juice_object_set_map_value(obj, key, object_to_juice_object(string_object));
}

bool juice_object_set_map_number(juice_object_t obj, const char *key, double number) {
    object_t number_object = object_make_number(number);
    return juice_object_set_map_value(obj, key, object_to_juice_object(number_object));
}

bool juice_object_set_map_bool(juice_object_t obj, const char *key, bool value) {
    object_t bool_object = object_make_bool(value);
    return juice_object_set_map_value(obj, key, object_to_juice_object(bool_object));
}

juice_object_t juice_object_get_map_value_with_value_key(juice_object_t obj, juice_object_t key) {
    return object_to_juice_object(object_get_map_value(juice_object_to_object(obj), juice_object_to_object(key)));
}

juice_object_t juice_object_get_map_value(juice_object_t object, const char *key) {
    gcmem_t *mem = object_get_mem(juice_object_to_object(object));
    if (!mem) {
        return juice_object_make_null();
    }
    object_t key_object = object_make_string(mem, key);
    ...
    juice_object_t res = juice_object_get_map_value_with_value_key(object, object_to_juice_object(key_object));
    return res;
}

const char* juice_object_get_map_string(juice_object_t object, const char *key) {
    juice_object_t res = juice_object_get_map_value(object, key);
    return juice_object_get_string(res);
}

double juice_object_get_map_number(juice_object_t object, const char *key) {
    juice_object_t res = juice_object_get_map_value(object, key);
    return juice_object_get_number(res);
}

bool juice_object_get_map_bool(juice_object_t object, const char *key) {
    juice_object_t res = juice_object_get_map_value(object, key);
    return juice_object_get_bool(res);
}

bool juice_object_map_has_key(juice_object_t juice_object, const char *key) {
    object_t object = juice_object_to_object(juice_object);
...
const char* juice_error_get_message(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
    return error->message;
}

const char* juice_error_get_filepath(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
    if (!error->pos.file) {
        return NULL;
    }
...
const char* juice_error_get_line(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
    if (!error->pos.file) {
        return NULL;
    }
...
int juice_error_get_line_number(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
    if (error->pos.line < 0) {
        return -1;
    }
...
int juice_error_get_column_number(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
...
juice_error_type_t juice_error_get_type(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
    switch (error->type) {
        case ERROR_NONE:        return JUICE_ERROR_NONE;
        case ERROR_PARSING:     return JUICE_ERROR_PARSING;
        case ERROR_COMPILATION: return JUICE_ERROR_COMPILATION;
        case ERROR_RUNTIME:     return JUICE_ERROR_RUNTIME;
        case ERROR_TIMEOUT:     return JUICE_ERROR_TIMEOUT;
        case ERROR_ALLOCATION:  return JUICE_ERROR_ALLOCATION;
        case ERROR_USER:        return JUICE_ERROR_USER;
        default:                return JUICE_ERROR_NONE;
    }
}

const char* juice_error_get_type_string(const juice_error_t *error) {
    return juice_error_type_to_string(juice_error_get_type(error));
}

const char* juice_error_type_to_string(juice_error_type_t type) {
    switch (type) {
        case JUICE_ERROR_PARSING:     return "PARSING";
        case JUICE_ERROR_COMPILATION: return "COMPILATION";
        case JUICE_ERROR_RUNTIME:     return "RUNTIME";
        case JUICE_ERROR_TIMEOUT:     return "TIMEOUT";
        case JUICE_ERROR_ALLOCATION:  return "ALLOCATION";
        case JUICE_ERROR_USER:        return "USER";
        default:                    return "NONE";
    }
}

char* juice_error_serialize(juice_t *juice, const juice_error_t *err) {
    const char *type_str = juice_error_get_type_string(err);
    const char *filename = juice_error_get_filepath(err);
    const char *line = juice_error_get_line(err);
    int line_num = juice_error_get_line_number(err);
    int col_num = juice_error_get_column_number(err);
    strbuf_t *buf = strbuf_make(&juice->alloc);
    if (!buf) {
        return NULL;
    }
    if (line) {
    ...
    strbuf_appendf(buf, "%s ERROR in \"%s\" on %d:%d: %s\n", type_str,
           filename, line_num, col_num, juice_error_get_message(err));
    const juice_traceback_t *traceback = juice_error_get_traceback(err);
    if (traceback) {
        strbuf_appendf(buf, "Traceback:\n");
        traceback_to_string((const traceback_t*)juice_error_get_traceback(err), buf);
    }
    if (strbuf_failed(buf)) {
        strbuf_destroy(buf);
        return NULL;
...
const juice_traceback_t* juice_error_get_traceback(const juice_error_t *juice_error) {
    const error_t *error = (const error_t*)juice_error;
    return (const juice_traceback_t*)error->traceback;
}

int juice_traceback_get_depth(const juice_traceback_t *juice_traceback) {
    const traceback_t *traceback = (const traceback_t*)juice_traceback;
    return array_count(traceback->items);
}

const char* juice_traceback_get_filepath(const juice_traceback_t *juice_traceback, int depth) {
    const traceback_t *traceback = (const traceback_t*)juice_traceback;
...
const char* juice_traceback_get_line(const juice_traceback_t *juice_traceback, int depth) {
    const traceback_t *traceback = (const traceback_t*)juice_traceback;
    traceback_item_t *item = array_get(traceback->items, depth);
    if (!item) {
        return NULL;
...
int juice_traceback_get_line_number(const juice_traceback_t *juice_traceback, int depth) {
    const traceback_t *traceback = (const traceback_t*)juice_traceback;
    traceback_item_t *item = array_get(traceback->items, depth);
    if (!item) {
        return -1;
...
int juice_traceback_get_column_number(const juice_traceback_t *juice_traceback, int depth) {
    const traceback_t *traceback = (const traceback_t*)juice_traceback;
    traceback_item_t *item = array_get(traceback->items, depth);
    if (!item) {
        return -1;
...
const char* juice_traceback_get_function_name(const juice_traceback_t *juice_traceback, int depth) {
    const traceback_t *traceback = (const traceback_t*)juice_traceback;
    traceback_item_t *item = array_get(traceback->items, depth);
    if (!item) {
        return "";
...
static void juice_deinit(juice_t *juice) {
    vm_destroy(juice->vm);
    compiler_destroy(juice->compiler);
    global_store_destroy(juice->global_store);
    gcmem_destroy(juice->mem);
    ptrarray_destroy_with_items(juice->files, compiled_file_destroy);
    errors_deinit(&juice->errors);
}

static object_t juice_native_fn_wrapper(vm_t *vm, void *data, int argc, object_t *args) {
    (void)vm;
    native_fn_wrapper_t *wrapper = (native_fn_wrapper_t*)data;
    JUICE_ASSERT(vm == wrapper->juice->vm);
    juice_object_t res = wrapper->fn(wrapper->juice, wrapper->data, argc, (juice_object_t*)args);
    if (juice_has_errors(wrapper->juice)) {
        return object_make_null();
    }
    return juice_object_to_object(res);
}
...
static juice_object_t object_to_juice_object(object_t obj) {
    return (juice_object_t){ ._internal = obj.handle };
}
...
static juice_object_t juice_object_make_native_function_with_name(juice_t *juice, const char *name, juice_native_fn fn, void *data) {
    ...
    wrapper.juice = juice;
    wrapper.data = data;
    object_t wrapper_native_function = object_make_native_function(juice->mem, name, juice_native_fn_wrapper, &wrapper, sizeof(wrapper));
    if (object_is_null(wrapper_native_function)) {
        return juice_object_make_null();
    }
    return object_to_juice_object(wrapper_native_function);
}
...
static void set_default_config(juice_t *juice) {
    memset(&juice->config, 0, sizeof(juice_config_t));
    juice_set_repl_mode(juice, false);
    juice_set_timeout(juice, -1);
    juice_set_file_read_function(juice, read_file_default, juice);
    juice_set_file_write_function(juice, write_file_default, juice);
    juice_set_stdout_write_function(juice, stdout_write_default, juice);
}

static char* read_file_default(void *ctx, const char *filename){
    juice_t *juice = (juice_t*)ctx;
    ...
    size_to_read = pos;
    rewind(fp);
    file_contents = (char*)allocator_malloc(&juice->alloc, sizeof(char) * (size_to_read + 1));
    if (!file_contents) {
        fclose(fp);
        return NULL;
    }
...
static void* juice_malloc(void *ctx, size_t size) {
    juice_t *juice = (juice_t*)ctx;
    void *res = allocator_malloc(&juice->custom_allocator, size);
    if (!res) {
        errors_add_error(&juice->errors, ERROR_ALLOCATION, src_pos_invalid, "Allocation failed");
    }
    return res;
}
...
static void juice_free(void *ctx, void *ptr) {
    juice_t *juice = (juice_t*)ctx;
    allocator_free(&juice->custom_allocator, ptr);
}
//FILE_END